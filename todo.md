...
### Critical Business Logic Updates (Jan 10)

- [x] Restrict ordering to T.Nagar only
  - [x] Hide Palladium option from order type selection
  - [x] Restore Pickup option for T.Nagar only
  - [x] Only show Dine-in, Pickup, and Delivery for T.Nagar
  
- [x] Fix pickup location address
  - [x] Changed to "Ground Floor New No.29 Burkit Road, TNagar, Chennai - 17"
  
- [x] Loyalty stamp system - Backend
  - [x] Calculate stamps: 1 stamp per ₹450 spent
  - [x] Track stamps earned per order (stampsEarned field added to orders table)
  - [x] Added stamp calculation to both guest and authenticated order creation
  
- [ ] Loyalty stamp system - Frontend
  - [ ] Show login prompt before checkout for loyalty stamps
  - [ ] Redeem 10 stamps for free large bubble tea
  - [ ] Show stamp count in user profile for registered customers
  - [ ] Reset stamp count after redemption
  
- [ ] Table number requirement
  - [ ] Make table number mandatory for dine-in orders
  - [ ] Prevent duplicate orders from same table number
  - [ ] Use table number as key for add-to-order functionality
  
- [ ] Payment method selection
  - [ ] Show "Pay at Counter" option for dine-in orders
  - [ ] When "Pay at Counter" selected, show payment method dialog
  - [ ] Payment methods: UPI, Credit/Debit Card, GPay/PhonePe, Zomato, Swiggy, EazyDiner
  - [ ] Only Razorpay payments processed through system
  - [ ] Other payments recorded manually by staff
  
- [ ] Implement delivery distance validation
  - [ ] Add 10km radius check for delivery orders
  - [ ] Show popup if address is outside delivery range
  - [ ] Allow customer to book Porter for out-of-range deliveries
  
- [ ] Business hours - Soft warning for dine-in after 11:45 PM
  - [ ] T.Nagar: 12 noon to 12 midnight
  - [ ] Show soft warning for dine-in orders after 11:45 PM (not blocking)
  - [ ] Still allow order submission with warning
