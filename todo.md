...
## Payment UI Simplification (Feb 16)

- [x] Remove Walkout button from order list — not requested, overcomplication
- [x] Make Collect Payment the clear primary action for staff to mark payment as collected
- [x] Show payment collected status clearly in order list (who collected, when)
- [x] Fix duplicate DB columns (payment_collected_by vs paymentCollectedBy) — dropped snake_case duplicates
- [x] Fix paymentCollectedBy schema from int to varchar(255) — stores staff name not user ID
- [x] Fix verifyRazorpayPayment UPDATE query — was passing user ID (number) instead of name (string)
- [x] Unified Collect Payment button for ALL order types (instore, delivery, pickup)
- [x] Added Collect Payment + Verify Razorpay buttons to Staff Orders page
- [x] Added Collect Payment + Verify Razorpay buttons to order detail dialog
- [x] Updated Tables tab to use confirmPaymentManually with staff accountability
- [x] Fixed Drizzle migration journal — marked 29 already-applied migrations as applied
- [x] 27 vitest tests passing for payment tracking logic
