...
export const appRouter = router({
  ...
  auth: router({
    ...
    logout: publicProcedure.mutation(() => {
      // Clerk manages sessions client-side via clerk.signOut()
      // This mutation remains for backward compatibility with useAuth hook
                ...
                item.withBoba !== null && item.withBoba !== undefined ? (
                  item.withBoba 
                    ? (item.bobaType === 'popping' 
                        ...
                        : `Boba: Tapioca${item.bobaSize ? ` (${item.bobaSize})` : ''}`)
                    : 'Boba: No'
                ) : null,
